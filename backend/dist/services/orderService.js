...
export async function evaluateDualTrigger(orderId) {
    ...
    if (tableReady && customerOnMyWay && order.status === 'confirmed') {
        ...
        if (entry) {
            notifyOrderCooking(entry.customer.phone, entry.customer.name).catch((err) => console.error('Failed to send cooking WhatsApp notification:', err));
        }
        return order;
    }
    return order;
}
export async function createPreOrder(queueEntryId, items) {
    const entry = await QueueEntry.findById(queueEntryId);
    if (!entry)
        throw new Error('Queue entry not found');
    if (entry.preOrderId) {
        const existingOrderId = typeof entry.preOrderId === 'object' ? entry.preOrderId._id : entry.preOrderId;
