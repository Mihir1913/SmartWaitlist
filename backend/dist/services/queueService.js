...
function sortQueuePriority(entries) {
    return entries.sort((a, b) => {
        const aVip = a.isVip ? 0 : 1;
        const bVip = b.isVip ? 0 : 1;
        if (aVip !== bVip)
            return aVip - bVip;
...
export async function calculateETA(restaurantId) {
...
export async function recalculateQueuePositions(restaurantId) {
...
export async function joinQueue(restaurantId, data) {
    const restaurant = await Restaurant.findById(restaurantId);
    if (!restaurant)
...
export async function markOnMyWay(entryId) {
...
export async function getQueue(restaurantId) {
...
export async function getNextInQueue(restaurantId) {
