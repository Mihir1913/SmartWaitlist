...
import { WhatsAppLog, WhatsAppMessageStatus } from '../models/WhatsAppLog.js';
...
export class WhatsAppService {
   ...
   */
  static async getCredentials(restaurantId?: string): Promise<MetaWhatsAppCredentials> {
    if (restaurantId) {
   ...
   */
  static async postToMetaCloudApi(
    credentials: MetaWhatsAppCredentials,
    payload: Record<string, unknown>,
    retries = 3
  ): Promise<{ messageId?: string; responseData?: unknown }> {
  ...
  static async sendTemplateMessage(
    ...
    try {
      ...
      logEntry.response = responseData as Record<string, unknown>;
      await logEntry.save();
   ...
   */
  static async sendTextMessage(to: string, textBody: string, restaurantId?: string) {
    const formattedTo = this.formatPhoneNumber(to);
    const credentials = await this.getCredentials(restaurantId);
   ...
   */
  static async sendWaitlistJoinConfirmation(
    phone: string,
    customerName: string,
    position: number,
    estWait: number,
   ...
   */
  static async sendRestaurantWelcome(
    phone: string,
    customerName: string,
   ...
   */
  static async updateMessageStatus(metaMessageId: string, status: WhatsAppMessageStatus, errorReason?: string) {
    const log = await WhatsAppLog.findOne({ metaMessageId });
    if (log) {
      log.status = status;
      if (errorReason) log.error = errorReason;
      await log.save();
