...
import { z } from 'zod';
import { joinQueue, getQueue, markOnMyWay, cancelQueueEntry } from '../services/queueService.js';
...
router.get('/entry/:entryId', async (req, res) => {
  try {
    const rawParam = (req.params.entryId as string).trim();
    ...
    if (!entry && searchParam.length >= 4) {
      ...
      entry =
        allActive.find((e) =>
          e._id.toString().toLowerCase().endsWith(searchParam.toLowerCase())
        ) || null;
    }
...
router.get('/:restaurantId', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const queue = await getQueue(req.params.restaurantId as string);
    res.json({ queue });
  } catch {
...
router.patch('/:entryId/on-my-way', async (req, res) => {
  try {
    const entry = await markOnMyWayForOrder(req.params.entryId as string);
    res.json({ entry });
  } catch (err) {
...
router.patch('/:entryId/cancel', async (req, res) => {
  try {
    const entry = await cancelQueueEntry(req.params.entryId as string);
    res.json({ entry });
  } catch (err) {
...
router.post('/:entryId/pre-order', async (req, res) => {
  try {
    ...
    const order = await createPreOrder(req.params.entryId as string, items);
