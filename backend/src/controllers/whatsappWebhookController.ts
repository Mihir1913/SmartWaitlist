  ...
  static async verifyWebhook(req: Request, res: Response) {
    try {
      ...
      if (!isVerified) {
        ...
        if (matchingSetting) isVerified = true;
      }
            ...
            for (const msg of value.messages) {
              ...
              if (joinMatch) {
                ...
                if (restaurant) {
                  ...
                  await WhatsAppService.sendWaitlistJoinConfirmation(
                    fromPhone,
                    customerName,
                    qEntry.position,
                    qEntry.estimatedWaitMinutes,
                ...
                if (activeEntry?.restaurantId) {
                  await WhatsAppService.sendTextMessage(
                    fromPhone,
                    `Thanks! We have marked you as 'On My Way'. Your table will be ready shortly!`,
                    activeEntry.restaurantId.toString()
                  );
                }
              }
            }
